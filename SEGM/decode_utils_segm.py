...
the decode_segm.py script and the SGNMT software architecture consisting of
decoders, predictors, and output handlers.

This script follows SGNMT decoder utils cam.sgnmt.decode_utils.py but adapted for morphological segmentation (cED+LM): the functionality is reduced to
--> predictors WordCountPredictorSegm,Word2charPredictorSegm,SRILMPredictorSegm (see predictors.py)
--> decoders BeamDecoderSegm,SyncBeamDecoderSegm (see decoders.py)
...
#                                           tf_get_rnnlm_prefix())
                ...
                p = WordCountPredictorSegm(args.wc_word,args.sync_symbol)
...
def create_decoder(new_args):
    ...
    if args.decoder == "greedy":
        ...
        decoder = BeamDecoderSegm(args,args.hypo_recombination,args.beam,args.pure_heuristic_scores,args.decoder_diversity_factor,args.early_stopping)
...
#                                      args.max_word_len)
        ...
        decoder = SyncBeamDecoderSegm(args,args.hypo_recombination,args.beam,args.pure_heuristic_scores,args.decoder_diversity_factor,args.early_stopping,args.sync_symbol,args.max_word_len)
...
#                logging.info("Stats (ID: %d): score=<not-found> "
                logging.debug("Stats (ID: %d): score=<not-found> "
                         "num_expansions=%d "
                         "time=%.2f" % (sen_idx+1,
