...
public class MertCore
{
  ...
  private int runWithExtDict; //Important for synchronising number of parameters to decode and rerank
  ...
  private String decoderConfigFileName, decoderCommandFileName, decoderCommandFileName_TEST, evalCommandFileName_TEST;
...
//      printMemoryUsage();
      ...
      String[] decRunResult = run_decoder(iteration, lambda); // iteration passed in case fake decoder will be used
...
//      println("",2);
  ...
  private String[] run_decoder_old(int iteration)
  {
...
*/
    ...
    private String[] run_decoder(int iteration, double[] params)
    {
        String[] retSA = new String[2];
        // [0] name of file to be processed
        // [1] indicates how the output file was obtained:
        ...
        if (fakeFileNameTemplate != null && fileExists(fakeFileNamePrefix+iteration+fakeFileNameSuffix)) {
            ...
            try {
                ...
                if (!decoderCommandFileName.equals("./SDecoder_cmd_old"))
                 {
                
                DecimalFormat df = new DecimalFormat("#.000000");
                
                BufferedReader br_decoder_dev = new BufferedReader(new FileReader(decoderCommandFileName));
                String b_cmd = br_decoder_dev.readLine();
                
                String weights_to_pass = " --predictor_weights "+df.format(params[1]);
                for (int c = 2; c <= numParams; ++c) {weights_to_pass = weights_to_pass + "," + df.format(params[c]);}
                b_cmd = b_cmd + weights_to_pass;
                
                    
                if (passIterationToDecoder == 1) {
                    b_cmd = b_cmd + " " + iteration;
                }
                    
                
                FileOutputStream outStream_dec_dev = new FileOutputStream(decoderCommandFileName+".temp");
                ...
                Process p_chmod = rt.exec("chmod 755 "+cmd);
                p_chmod.waitFor();
                Process p = rt.exec(cmd);
...
*/
        ...
        } else if (option.equals("-cmdTest")) {
                decoderCommandFileName_TEST = args[i+1];
        } else if (option.equals("-evalTest")) {
                evalCommandFileName_TEST = args[i+1];
            
        } else if (option.equals("-runWithExtDictFeat")) {
            runWithExtDict = Integer.parseInt(args[i+1]);
...
//      DMC.printMemoryUsage();
    ...
    //redecode and run evaluation on the test data
        try {
            Runtime rt1 = Runtime.getRuntime();
            
            String cmd1 = DMC.decoderCommandFileName_TEST;
            
            if (!DMC.decoderCommandFileName_TEST.equals("./Rerank_segm_cmd"))
            {
            
            BufferedReader br_decoder = new BufferedReader(new FileReader(DMC.decoderCommandFileName_TEST));
            String b_cmd1 = br_decoder.readLine();
            
            
            DecimalFormat df = new DecimalFormat("#.000000");
            
            
            String weights_to_pass = " --predictor_weights "+ df.format(DMC.lambda[1]);
                
            int weights_number = DMC.numParams;
            if (DMC.runWithExtDict == 1)
            {weights_number = DMC.numParams-1;}
                
                
            for (int c = 2; c <= weights_number; ++c) {weights_to_pass = weights_to_pass + "," + df.format(DMC.lambda[c]);}
            b_cmd1 = b_cmd1 + weights_to_pass;
            
            FileOutputStream outStream_dec = new FileOutputStream(DMC.decoderCommandFileName_TEST+".temp");
            ...
            Process p1_chmod = rt1.exec("chmod 755 "+cmd1);
            p1_chmod.waitFor();
            Process p1 = rt1.exec(cmd1);
            
            StreamGobbler errorGobbler = new StreamGobbler(p1.getErrorStream(), DMC.decVerbosity);
            StreamGobbler outputGobbler = new StreamGobbler(p1.getInputStream(), DMC.decVerbosity);
            
            errorGobbler.start();
            outputGobbler.start();
            
            int decStatus = p1.waitFor();
            if (decStatus != DMC.validDecoderExitValue) {
                DMC.println("Call to decoder on the test set returned " + decStatus
                        + "; was expecting " + DMC.validDecoderExitValue + ".");
                System.exit(30);
        }
    } catch (IOException e) {
            System.err.println("IOException in MertCore.run_decoder(int): " + e.getMessage());
...
//            String cmd2 = DMC.evalCommandFileName_TEST;
//            DMC.println(cmd2);
//            
//            Process p2 = rt2.exec(cmd2);
//            
//            StreamGobbler errorGobbler = new StreamGobbler(p2.getErrorStream(), DMC.decVerbosity);
//            StreamGobbler outputGobbler = new StreamGobbler(p2.getInputStream(), DMC.decVerbosity);
//            
//            errorGobbler.start();
//            outputGobbler.start();
//            
//            int decStatus = p2.waitFor();
//            if (decStatus != DMC.validDecoderExitValue) {
//                DMC.println("Call to evaluation returned " + decStatus
//                        + "; was expecting " + DMC.validDecoderExitValue + ".");
//                System.exit(30);
//            }
//        } catch (IOException e) {
